#include "FetchUserInputWidget.h"
#include "InputContainer.h"

FetchUserInputWidget::FetchUserInputWidget(const InputContainer* input, QWidget* parent) :
    QDialog(parent),
...
FetchUserInputWidget::~FetchUserInputWidget()
{

}

void FetchUserInputWidget::mousePressEvent(QMouseEvent* event)
{
...
void FetchUserInputWidget::keyPressEvent(QKeyEvent* event)
{
    recordedModifier = event->modifiers();
...
void FetchUserInputWidget::processPressedInput(int mod, int key)
{
...
bool FetchUserInputWidget::hasDuplicateBinding(const int mod, const int key)
{
    return (inputContainer->doesExist(mod, key));
}

int FetchUserInputWidget::convertKey(int key)
{
    switch(key)
    {
    case 33: { return '1'; }
...
QString FetchUserInputWidget::convertText(QString text, int key)
{
    switch(key)
    {
    case 1:
