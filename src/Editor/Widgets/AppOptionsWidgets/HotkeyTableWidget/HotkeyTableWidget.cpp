...
HotkeyTableWidget::~HotkeyTableWidget()
{

}

void HotkeyTableWidget::addTable(QString title, QTableWidget* table)
{
    this->addTab(table, title);
}

QTableWidget* HotkeyTableWidget::createTable(const InputContainer* inputContainer, const QMap<QString, InputKey*>& keys)
{
    int rows = keys.size();
    int cols = 2;
...
void HotkeyTableWidget::setupButtonConnect(QPushButton* button, const InputContainer* inputContainer, InputKey* key)
{
    connect(button, &QPushButton::clicked, key, [this, button, inputContainer, key]()
    {
        FetchUserInputWidget fetchUserInput(inputContainer);
        fetchUserInput.exec();
...
QString HotkeyTableWidget::convertText(QString text, int key)
{
    switch(key)
    {
    case 1:
